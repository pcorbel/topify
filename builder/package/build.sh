...
for docker_arch in ${TARGET_ARCHES}; do
  ...
  cp ${script_dir}/Dockerfile.cross ${script_dir}/Dockerfile.${docker_arch}
  sed -i  "s|__BASEIMAGE_ARCH__|${docker_arch}|g" ${script_dir}/Dockerfile.${docker_arch}
  sed -i  "s|__QEMU_ARCH__|${qemu_arch}|g" ${script_dir}/Dockerfile.${docker_arch}
  if [[ ${docker_arch} == "amd64" || ${build_os} == "darwin" ]]; then
    sed -i  "/__CROSS_/d" ${script_dir}/Dockerfile.${docker_arch}
  else
    sed -i  "s/__CROSS_//g" ${script_dir}/Dockerfile.${docker_arch}
  fi
  ${docker_bin_path} build -f ${script_dir}/Dockerfile.${docker_arch} -t ${REPO}/${IMAGE_NAME}:${docker_arch}-${IMAGE_VERSION} .
  ...
  rm ${script_dir}/Dockerfile.${docker_arch}
done
